...
exportBtn?.addEventListener("click", async () => {
  ...
  if (dueFrom && dueTo && dueFrom > dueTo) {
    window.alert("Due date From must be on or before To.");
    return;
  }
  ...
  try {
    const startResponse = await fetch("/api/export-missed-submissions/start", {
      method: "POST",
    ...
    a.download =
      latestJob?.fileName ||
      `missed_submissions_${categoryName.replace(/\s+/g, "_")}_${Date.now()}.xlsx`;
